...
export const EditQuiz: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        setError('Erro ao carregar o quiz: ' + err.message);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSaveQuestion = async () => {
    ...
    if (editingIndex !== null) {
      const target = questions[editingIndex];
      ...
      await api.question.update(target.id, updatedData);
      const updated = [...questions];
      updated[editingIndex] = { ...target, ...updatedData } as Question;
      ...
      const res = await api.question.create(newQ);
      setQuestions([...questions, { ...newQ, id: res.id } as Question]);
    }
  ...
  const handleDeleteQuestion = async (idx: number) => {
    ...
    if (confirm('Deseja excluir esta questão?')) {
      await api.question.delete(q.id);
  ...
  if (loading) {
    return (
      <div style={{ padding: '3rem', textAlign: 'center' }}>
        <p style={{ fontFamily: 'var(--font-heading)', fontSize: '1.25rem', fontWeight: 800 }}>Carregando dados do quiz...</p>
      </div>
    );
  }
    ...
    <div style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto', width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <NeoBrutalistButton variant="secondary" size="md" onClick={() => navigate(`/quiz/${id}/import`)}>
            📥 IMPORTAR QUESTÕES
          </NeoBrutalistButton>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {questions.map((q, idx) => (
          <NeoBrutalistCard key={q.id || idx} style={{ padding: '1.25rem' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxHeight: '70vh', overflowY: 'auto', paddingRight: '0.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>ALTERNATIVA A *</label>
              <input type="text" className="neo-input" value={optA} onChange={(e) => setOptA(e.target.value)} style={{ width: '100%', padding: '0.6rem', fontWeight: 600 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>ALTERNATIVA B *</label>
              <input type="text" className="neo-input" value={optB} onChange={(e) => setOptB(e.target.value)} style={{ width: '100%', padding: '0.6rem', fontWeight: 600 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>ALTERNATIVA C *</label>
              <input type="text" className="neo-input" value={optC} onChange={(e) => setOptC(e.target.value)} style={{ width: '100%', padding: '0.6rem', fontWeight: 600 }} />
            </div>
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>ALTERNATIVA D *</label>
              <input type="text" className="neo-input" value={optD} onChange={(e) => setOptD(e.target.value)} style={{ width: '100%', padding: '0.6rem', fontWeight: 600 }} />
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem', marginTop: '0.5rem' }}>
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>RESPOSTA CORRETA</label>
              <select className="neo-input" value={correctAnswer} onChange={(e) => setCorrectAnswer(e.target.value as any)} style={{ width: '100%', padding: '0.6rem', fontWeight: 800 }}>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>TEMPO</label>
              <select className="neo-input" value={timeSeconds} onChange={(e) => setTimeSeconds(Number(e.target.value))} style={{ width: '100%', padding: '0.6rem', fontWeight: 800 }}>
            ...
            <div>
              <label style={{ display: 'block', fontWeight: 800, marginBottom: '0.3rem' }}>PONTOS</label>
              <select className="neo-input" value={basePoints} onChange={(e) => setBasePoints(Number(e.target.value))} style={{ width: '100%', padding: '0.6rem', fontWeight: 800 }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontWeight: 700 }}>
              <input type="checkbox" checked={isWildcard} onChange={(e) => setIsWildcard(e.target.checked)} style={{ width: '18px', height: '18px' }} />
              ⚡ Questão de Virada (pontuação dobrada)
            </label>
          </div>

          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
            <NeoBrutalistButton variant="ghost" size="md" onClick={() => setIsModalOpen(false)}>CANCELAR</NeoBrutalistButton>
            <NeoBrutalistButton variant="primary" size="md" onClick={handleSaveQuestion}>SALVAR QUESTÃO</NeoBrutalistButton>
          </div>
        </div>
      </NeoBrutalistModal>
